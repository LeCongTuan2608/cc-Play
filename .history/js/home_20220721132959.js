...
slidePosterStart();
function renderPosterSlider(params, listSelector) {
   const listPosterSlider = document.querySelector(listSelector);
   if (!listMovie) {
   ...
   for (let i = 0; i < params.length; i++) {
      let randomNumber = Math.floor(Math.random() * 230) + 4;
      newarray.push(params[randomNumber])
   }
   console.log(newarray);
   let htmls = params.map((items) => {
      return `
      <li>
         <a href="">
   ...
   listPosterSlider.innerHTML = await htmls.join('');
...
async function getMovieInforFromApi(movie) {
   try {
      ...
      renderPosterSlider(arrayList, '.middle-page-mid-posters ul');
