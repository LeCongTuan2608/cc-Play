...
async function filterCategory(listMovie, nameCategory, listSelector, type) {
   ...
   for (let i = 0; i < lengthMovie; i++) {
      ...
      for (let j = 0; j < cateLength; j++) {
         const nameCountry = listMovie[i].movie.type;
         console.log(nameCountry);
         const countryLength = nameCountry.length;
         for (let t = 0; t < countryLength; t++) {
            if (cate[j].name == nameCategory && nameCountry[t].name != type) {
...
async function getMovieInforFromApi(movie) {
   try {
      ...
      filterCategory(arrayList, getCategory, '.category-movie');
   } catch (error) {
      console.error({ error });
   }
}
