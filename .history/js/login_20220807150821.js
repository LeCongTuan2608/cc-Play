...
//định nghĩa method
validate.isPassword = (selector, message) => {
   return {
      selector: selector,
      checked: (value) => {
         return value.trim() ? undefined : message || 'Vui lòng nhập password!';
      },
   };
};
...
validate({
   ...
   rules: [validate.isEmail('#email'), validate.isPassword('#password', 'Mật khẩu không chính xác!')],
});
