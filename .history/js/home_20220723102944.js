...
// lọc thể loại phim
const TheMovie = document.querySelectorAll('.movie-swiper-slider');
export async function filterCategory(listMovie, nameCategory, listSelector) {
   ...
   await renderListMovie(getArrMovies15, listSelector).then((arrTheMovies) => {
      console.log(arrTheMovies.length - 5);
   });
}
const test = async () => {
   await renderListMovie().then((arrTheMovies) => {
      for (let i = 0; i < arrTheMovies.length; i++) {
         const getArrMovies = arrTheMovies.slice(arrTheMovies.length - 9, arrTheMovies.length);
      }
      console.log(getArrMovies);
   });
};
test();
//fechAPI lần 2
