...
const renderTheMovie = async (movieDetail) => {
   ...
   let htmls = `
            <div class="video-container" eps = ${eps}>
               <iframe src="${urlVideo}"
...
const renderEpisode = async (movieDetail) => {
   ...
   epss.style.background = 'rgb(148, 190, 64)';
};
   ...
   for (let i = 0; i < epss.length; i++) {
      ...
      epss[i].onclick = async () => {
         for (let j = 0; j < epss.length; j++) {
            epss[j].style.background = '#7e7e7e';
         }
         epss[i].style.background = 'linear-gradient(132deg, rgb(241, 214, 105) 0%, rgb(30, 164, 137) 100%)';
         if (getEps == getEpsVideo) {
