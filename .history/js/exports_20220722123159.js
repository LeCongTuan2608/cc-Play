...
export const renderListMovie = async (data, listSelector) => {
   ...
   await hideLoading();
   const movies = document.querySelectorAll('.container-swiper');
   arrMovies.push(movies);
   await getMovie(arrMovies);
};

export async function getMovie(arrMovies) {
   const lengthMovie = await arrMovies.length;
   for (let i = 0; i < lengthMovie; i++) {
      arrMovies[i].onclick = async () => {
         console.log(arrMovies[i]);
         localStorage.setItem('getMovieDetails', JSON.stringify(arrMovies));
      };
   }
}
