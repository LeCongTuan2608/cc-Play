import { url_page, url_movie } from './exports.js';
...
const ChooseCategory = document.querySelectorAll('.menu-category ul li');
const vlue = ChooseCategory.forEach((items) => {
   ChooseCategory.onclick = () => {
      alert('ban vua click');
   };
});
// const clickCate = category.addEventListener('click', () => {
...
async function getMovieInforFromApi(movie) {
   try {
      const response = await fetch(`${url_movie + movie}`);
...
async function getMoviePageFromApi(page, listSelector) {
   ...
   try {
      const response = await fetch(`${url_page + page}`);
