...
validate.isEmail = (selector, message, getEmail) => {
   return {
      ...
      checked: (value) => {
         ...
         return regex.test(value) == getEmail ? undefined : 'Vui lòng nhập email!';
      },
   };
};
...
validate.isPassword = (selector, message, getPassword) => {
   return {
      ...
      checked: (value) => {
         ...
         return value.trim() == getPassword ? undefined : mess;
      },
   };
};
...
validate({
   ...
   rules: [
      validate.isEmail('#email', 'Email này không đúng!', 'email@gmail.com'),
