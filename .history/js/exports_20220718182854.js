      ...
      <div class="container-swiper">
         <div class="movie-swiper-slider">
            <div class="posters-movie ">
               <a href="movie.html"><img class src="${items.movie.poster_url}" alt="Ảnh bị lỗi rồi :("></a>
            </div>
...
// lọc thể loại phim
import { arrayList } from './fechAPI.js';
export async function filterCategory(arrayList, nameCategory, listSelector, country, amount) {
   // amount số lượng hiện thị movie
   const arrayListMovie = [];
   const lengthMovie = arrayList.length;
   let count = 0;
   for (let i = 0; i < lengthMovie; i++) {
      const cate = arrayList[i].movie.category;
      const cateLength = cate.length;
      for (let j = 0; j < cateLength; j++) {
         const nameCountry = arrayList[i].movie.country;
         ...
         for (let t = 0; t < countryLength; t++) {
            if (cate[j].name == nameCategory && nameCountry[t].name != country) {
               count++;
               if (count < 15) {
                  arrayListMovie.push(arrayList[i]);
               }
