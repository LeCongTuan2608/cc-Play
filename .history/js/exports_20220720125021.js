...
export const ListCategory = document.querySelectorAll('.category ul li a');
export async function getCategory() {
   for (let i = 0; i < ListCategory.length; i++) {
      ListCategory[i].onclick = async () => {
         localStorage.setItem('getCategory', JSON.stringify(ListCategory[i].textContent));
      };
   }
}
getCategory();
