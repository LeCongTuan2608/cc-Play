   ...
   for (let i = 0; i < lengthMovie; i++) {
      ...
      for (let j = 0; j < cateLength; j++) {
         ...
         for (let t = 0; t < countryLength; t++) {
            for (let k = i + 1; k < lengthMovie; k++) {
               if (listMovie[i].movie._id === arrayList[k].movie._id) {
                  listMovie.splice(k, 1);
               } else if (cate[j].name == nameCategory && nameCountry[t].name != country) {
                  count++;
                  if (count < 15) {
